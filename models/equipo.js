...
class tModelo {
    ...
    Actualizar(i,r){
        return new Promise ((resolve,reject)=>{           
            var query = connection.query('UPDATE EQUIPOS SET  NOMBRE = ?, SERIAL = ?, DESCRIPCION = ?, FECHA = ?, ESTADO = ? WHERE  ID = ?', [r.NOMBRE, r.FECHA,r.SERIAL , r.DESCRIPCION,r.FECHA,r.ESTADO, i], function (error, results, fields) {
                if (error) throw error;
